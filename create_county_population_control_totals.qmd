...
**Only run chunks in this section when NYS DOL population data have been updated**
...
df2 <- df1 |> 
  mutate(area = str_remove(area, " County"),
    program = case_when(str_detect(program, "Census Base") ~ "censusbase",
...
saveRDS(df2, fs::path(DDATA, "cenpop_nysdol.rds"))
...
df3 <- df2 |> 
  mutate(
    rectype = ifelse(county == "000", "state", "county"),
    ctyname = str_remove(stname, " County")) |> 
  select(
    year,
    rectype,
    stfips = state, cofips = county,
    stname, coname = ctyname,
    variable, value)
