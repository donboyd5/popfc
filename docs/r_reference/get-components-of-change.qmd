...
CAUTION: The Census Bureau produces postcensal components of change (births, deaths, net migration) where initial year + components foot to subsequent year. After a new decennial census, they construct intercensal estimates that smooth the path for TOTAL population from one decennial census to the next. However, they do not prepare revised components of change that fit with these smoothed estimates. Thus, we do not have data that provide internally consistent components of change that walk from one decennial census to the next.

## Definitions

See: The Components of Demographic Change: When Does Each Matter Most?, Saswathi Natta and Daniel Goodkind, Released May 2024 (In folder)

- Crude birth rate (CBR) is defined as the number of births during a year per 1,000 population **at midyear**.
- Crude death rate (CDR) is defined as the number of deaths during a year per 1,000 population **at midyear**.
...
## Adjust births, deaths, and resid

```{r}
#| label: adjust-births-deaths-resid

# calculate improved births and deaths for decennial census years
coc7 <- coc6 |>
  mutate(
    avgpop = (pop + poplag) / 2,
    abirths = avgpop * rbirths / 1000,
    adeaths = avgpop * rdeaths / 1000,
    anatchg = abirths - adeaths,
    aresid = npopchg - anatchg - netmig,
  ) |>
  relocate(avgpop, .after = poplag) |>
  relocate(abirths, .after = births) |>
  relocate(adeaths, .after = deaths) |>
  relocate(aresid, .after = resid)

```

## Save final components of change file

```{r}
saveRDS(coc7, fs::path(DWORK, "cocwide.rds"))
```

## Explore

```{r}
#| label: check-adjustments

cnty <- "Albany" # Cayuga Columbia Genesee Greene Livingston Madison Montgomery Ontario Oswego Rensselaer Saratoga Schoharie Warren Washington Wyoming
df <- coc7 |>
  ...
  pivot_longer(-c(stfips, cntyfips, shortname, year))

cvar <- "resid" # deaths births resid
df |>
  filter(str_detect(name, cvar)) |>
  ggplot(aes(year, value, colour = name)) +
  geom_point() +
  geom_line() +
  geom_hline(yintercept = 0) + # use for resid
  ggtitle(paste(cnty, cvar))
...
tmp <- cocraw |>
  filter(stfips == "01", cntyfips == "001", year %in% c(2000, 2009, 2010)) |>
  filter(variable %in% c("popestimate", "censuspop", "estimatesbase"))
tmp

coc <- readRDS(fs::path(DWORK, "cocwide.rds"))
glimpse(coc)


cvar <- "deaths" # deaths netmig resid dmig imig pop
